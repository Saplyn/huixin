...
impl PatchNodeProcessable<'_> for RemoteData {
    ...
    fn process(node_id: NodeId, snarl: &mut Snarl<PatchNode>, _: Self::ProcessArg) -> bool {
        let PatchNode::RemoteData(remote) = &snarl[node_id] else {
            unreachable!();
        };

        let mut tag = None;
        if let Some(src) = remote.input_for_pin(RemoteData::INPUT_TAG) {
            tag = snarl[src.node].output_text(src.output);
        }
        let mut prop = None;
        if let Some(src) = remote.input_for_pin(RemoteData::INPUT_PROP) {
            prop = snarl[src.node].output_text(src.output);
        }

        let PatchNode::RemoteData(remote) = &mut snarl[node_id] else {
            unreachable!();
        };
