...
impl PatchNodeProcessable<'_> for Oscillator {
    ...
    fn process(
        ...
        let PatchNode::Oscillator(osc) = node_or_bail!(snarl, node_id) else {
            unreachable!();
        };

        let mut freq = None;
        if let Some(src) = osc.input_for_pin(Oscillator::INPUT_FREQ) {
            freq = node_or_bail!(snarl, src.node)
        ...
        if let Some(src) = osc.input_for_pin(Oscillator::INPUT_PHASE) {
            phase = node_or_bail!(snarl, src.node)
                .output_number(src.output)
                .map(|n| {
                    n.clamp(
        ...
        if let Some(src) = osc.input_for_pin(Oscillator::INPUT_WAVEFORM) {
            waveform = node_or_bail!(snarl, src.node)
                .output_number(src.output)
                .map(|n| {
                    n.clamp(
        ...
        if let Some(src) = osc.input_for_pin(Oscillator::INPUT_RESET) {
            reset = node_or_bail!(mut snarl, src.node).output_bang(src.output, node_id);
        }

        let PatchNode::Oscillator(osc) = node_or_bail!(mut snarl, node_id) else {
            unreachable!();
        };
